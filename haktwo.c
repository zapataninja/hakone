...
#pragma config(Motor, port3, M3, tmotorVex393_MC29, openLoop, encoderPort,     \
               I2C_2)
#pragma config(Motor, port5, M5, tmotorVex393_MC29, openLoop, encoderPort,     \
               I2C_1)
...
/* Original -
 * https://www.vexforum.com/index.php/6465-a-pid-controller-in-robotc*/
/*-----------------------------------------------------------------------------*/
/*																																						 */
/*	pid control task
 */
...
task pidController() {
  ...
  while (true) {
    ...
    pidError = sqrt((pidXError * pidXError) +
                    (pidYError * pidYError)); // SQRT(X2 + Y2)
    ...
    sprintf(string1, "%2.2f, %2.2f, %2.2f", pidError, pidIntegral,
            pidDerivative);
...
task get_sensor_data() {
  ...
  while (1) {
    ...
    clearLCDLine(1);
    // send to motor use pidDrive
    sprintf(string1, "%d, %d, %d", SensorValue(linefollower), nMotorEncoder[M3],
            nMotorEncoder[M5]);
    sprintf(string2, "%d, %d, %d", SensorValue(sonar_front),
            SensorValue(sonar_back), SensorValue(sonar_side));
    displayLCDString(0, 0, string1);
...
task main() {
  ...
  float rotationInDegrees =
      90 - ((atan2((pidRequestedYValue - pidCurrentYValue),
                   (pidRequestedXValue - pidCurrentXValue))) *
            180 / PI);
  float pidpostRotationXValue =
      pidCurrentXValue * cosDegrees(rotationInDegrees);
